...
} from '../../models/router-scroll.model';
...
export class RouterScrollService implements IRouterScrollService, OnDestroy {
  ...
  constructor(
    ...
    const scrollPositionRestore$ = this.router.events.pipe(
      filter(
        (event: any) => event instanceof NavigationStart || event instanceof NavigationEnd
      ),
      // Accumulate the scroll positions
      scan<NavigationEnd | NavigationStart, IScrollPositionRestore>((acc, event) => {
        if (environment.traceRouterScrolling) {
        ...
        const retVal: IScrollPositionRestore = {
          ...
          positions,
          trigger:
            event instanceof NavigationStart ? event.navigationTrigger : acc.trigger,
          idToRestore:
      ...
      filter((scrollPositionRestore: IScrollPositionRestore) =>
        Boolean(scrollPositionRestore.trigger)
      ),
        ...
        if (scrollPositionRestore.event instanceof NavigationEnd) {
          ...
          const imperativeTrigger =
            (scrollPositionRestore.trigger &&
              scrollPositionRestore.trigger === 'imperative') ||
            false;
          ...
          if (shouldScrollToTop) {
            ...
            if (this.customViewportToScroll) {
              this.customViewportToScroll.scrollTop =
                scrollPositionRestore.positions[
                  `${scrollPositionRestore.idToRestore}-${customViewportKey}`
                ];
            }
          }
  ...
  processAddQueue(queue: any): void {
  ...
  processRemoveQueue(queue: any, removeOnceBeforeNavigation = false): void {
  ...
  routeStrategyPosition(partialRoute: string): number {
